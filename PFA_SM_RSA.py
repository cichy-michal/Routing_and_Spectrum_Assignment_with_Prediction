...
class ElasticOpticalNetwork:
        ...
        for i in range(self.num_nodes):
            for j in range(self.num_nodes):
                if self.topology[i][j] > 0:
                    ...
                    self.spectrum_state[(j, i)] = [0] * self.num_fs
    ...
    def allocate_spectrum(self, path, start_fs, num_fs, connection_id):
        for i in range(len(path) - 1):
            ...
            link = (u, v)
            for slot in range(start_fs, start_fs + num_fs):
                self.spectrum_state[link][slot] = connection_id
    ...
    def phase1_strict_exact_fit(self, path, required_fs):
        for start_fs in range(self.num_fs - required_fs + 1):
            if not self.check_spectrum_availability(path, start_fs, required_fs):
                continue
            left_occupied = (start_fs == 0)
            right_occupied = (start_fs + required_fs == self.num_fs)
            if not left_occupied:
                for i in range(len(path) - 1):
                    u, v = path[i], path[i+1]
                    if self.spectrum_state[(u, v)][start_fs - 1] != 0:
                        left_occupied = True
                        break
            if not right_occupied:
                for i in range(len(path) - 1):
                    u, v = path[i], path[i+1]
                    if self.spectrum_state[(u, v)][start_fs + required_fs] != 0:
                        right_occupied = True
                        break
    ...
    def pfa_sm_rsa_algorithm(self, source, destination, bitrate):
    ...
    def process_request(self, source, destination, bitrate, duration):
        success, path, start_fs, num_fs = self.pfa_sm_rsa_algorithm(source, destination, bitrate)
        if success:
...
def simulate_instance(network, instance):
    ...
    print(f"Rozpoczynanie symulacji dla instancji {instance.folder_name}")
    for iteration in instance.iterations:
        ...
        for request in requests:
            ...
            success, _ = network.process_request(source, destination, bitrate, duration)
            if not success:
...
if __name__ == "__main__":
    ...
    for instance in instances:
        ...
        network = ElasticOpticalNetwork(cities, topology, num_nodes, num_links)
        instance_results = simulate_instance(network, instance)
        results[instance.folder_name] = instance_results
        end_time = time.time()
        execution_time = end_time - start_time
        instance_results['execution_time'] = execution_time
        print(f"\nWyniki dla instancji {instance.folder_name}:")
        print(f"  Czas wykonania: {execution_time:.2f} s")
        print(f"  Łączna liczba żądań: {instance_results['total_requests']}")
        print(f"  Zablokowane żądania: {instance_results['blocked_requests']}")
        print(f"  Prawdopodobieństwo blokady (BP): {instance_results['blocking_probability']:.4f}")
        print(f"  Średnie wykorzystanie widma: {instance_results['avg_spectrum_utilization']:.4f}")